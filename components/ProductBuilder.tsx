...
// Added Settings and Barcode to imports from lucide-react
import { ArrowRight, ArrowLeft, CheckCircle2, Cpu, ShieldCheck, Zap, Info, ChevronLeft, ChevronRight, X, Upload, Activity, Package, MessageCircle, Server, FileCheck, FileWarning, Layers, Search, BarChart3, Globe, FileDigit, Monitor, LayoutGrid, Clock, Shield, Settings, Barcode } from 'lucide-react';
...
const ProductBuilder: React.FC<ProductBuilderProps> = ({ onAddOrder, user, hubs, products, language }) => {
  ...
  // Defined missing isAdmin and isB2B constants based on user role
    ...
    <div className="max-w-[1600px] mx-auto px-6 py-12 industrial-grid min-h-screen pt-32 lg:pt-40">
      {step === 1 && (
         <div className="space-y-20 animate-in fade-in">
            {/* Catalog Header with Dynamic Stats */}
            <div className="flex flex-col xl:flex-row justify-between items-end gap-12">
                 ...
                 <div className="flex overflow-x-auto gap-3 pb-4 scrollbar-hide">
                    {categories.map(cat => (
                      <button 
                        ...
                        className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border ${activeCategory === cat ? 'bg-red-600 text-white border-red-600 shadow-xl' : 'bg-white text-gray-400 border-gray-100 hover:border-black hover:text-black'}`}
                      >
                        {cat}
                      </button>
                    ))}
               ...
               <div className="flex flex-col items-end gap-8 w-full max-w-xl">
                  ...
                  <div className="bg-white p-2 rounded-[2.5rem] border border-gray-100 shadow-2xl flex items-center w-full focus-within:border-black transition-all">
                     ...
                     <input 
                      type="text" 
                      placeholder="SEARCH PRODUCT GRID..." 
                      value={searchQuery} 
                      onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} 
                      className="bg-transparent flex-grow outline-none font-black uppercase text-[11px] p-6 placeholder:text-gray-200" 
                     />
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-12">
               {paginatedProducts.map(p => (
                 <div 
                    ...
                    className={`bg-white border-2 rounded-[4.5rem] p-10 transition-all cursor-pointer relative group ${selectedProduct?.id === p.id ? 'border-red-600 shadow-[0_50px_100px_rgba(204,0,0,0.1)] scale-[1.03]' : 'border-gray-50 hover:border-black shadow-lg'}`}
                 >
                    {p.badge && (
                      <div className="absolute top-12 right-12 bg-black text-white px-4 py-1.5 rounded-full text-[8px] font-black z-10 shadow-lg tracking-widest">{p.badge}</div>
                    )}
                    ...
                    <div className="w-full aspect-square rounded-[3.5rem] overflow-hidden mb-10 shadow-inner bg-gray-50 relative">
                       ...
                       {/* Quick Access Info on Image Hover */}
                       <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0">
                          <button onClick={(e) => { e.stopPropagation(); setDetailProduct(p); }} className="bg-white/90 backdrop-blur-md p-3 rounded-2xl text-black hover:bg-red-600 hover:text-white transition-all">
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="flex justify-between items-center mt-10 pt-6">
                          {user || isAdmin || isB2B ? (
                             <span className="text-4xl font-brand font-black italic">€{p.basePrice}<span className="text-[10px] font-normal opacity-30">/{p.unit}</span></span>
                          ) : (
                             <span className="text-[10px] font-black uppercase text-gray-300 italic tracking-[0.2em]">Login for pricing</span>
                          )}
                          <div className={`p-5 rounded-2xl transition-all ${selectedProduct?.id === p.id ? 'bg-red-600 text-white animate-pulse shadow-xl shadow-red-600/30' : 'bg-gray-50 text-gray-200 group-hover:bg-black group-hover:text-white'}`}>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center space-x-12 mt-20 py-10 border-t border-gray-100">
                 <button 
                  ...
                  className="p-6 bg-white border border-gray-100 rounded-full disabled:opacity-20 hover:bg-black hover:text-white transition-all shadow-xl"
                 >
                 ...
                 <div className="flex items-center space-x-4">
                    {Array.from({ length: totalPages }).map((_, i) => (
                      <button 
                        ...
                        className={`w-14 h-14 rounded-2xl font-brand font-black italic text-lg transition-all ${currentPage === i + 1 ? 'bg-red-600 text-white shadow-2xl scale-110' : 'bg-gray-50 text-gray-300 hover:bg-black hover:text-white'}`}
                      >
                        {i + 1}
                      </button>
                    ))}
            ...
            {selectedProduct && (
              <button 
                onClick={() => setStep(2)} 
                className="w-full bg-black text-white p-12 rounded-[4.5rem] font-brand font-black italic uppercase tracking-[0.6em] text-xl mt-12 hover:bg-red-600 transition-all shadow-2xl flex items-center justify-center animate-in slide-in-from-bottom-5"
              >
                Configurar Unidade R2 <ArrowRight className="ml-8 w-8 h-8 animate-pulse" />
              </button>
            )}
         </div>
      )}
      ...
      {step === 2 && (
         <div className="bg-white rounded-[5rem] shadow-2xl border border-gray-100 p-20 animate-in slide-in-from-right-10 relative overflow-hidden">
               ...
               <div className="space-y-12">
                  ...
                  <div className="space-y-4">
                    ...
                    <input 
                      ...
                      className="w-full bg-gray-50 p-8 rounded-[2.5rem] font-brand font-black italic text-2xl outline-none border border-transparent focus:border-red-600 shadow-inner" 
                    />
                  </div>

                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em] ml-6">Industrial Substrate</label>
                    <select value={config.material} onChange={e => setConfig({...config, material: e.target.value})} className="w-full bg-gray-50 p-8 rounded-[2.5rem] font-black uppercase text-[12px] outline-none border border-transparent focus:border-red-600 shadow-inner appearance-none cursor-pointer">
            ...
            {isSyncing ? (
              <div className="w-full mt-24 p-16 bg-black rounded-[4rem] flex flex-col items-center justify-center space-y-6 animate-pulse border-l-[20px] border-red-600">
                <Cpu className="w-16 h-16 text-red-600 animate-spin" />
                <span className="text-white font-brand font-black italic uppercase text-2xl tracking-tighter">Sincronizando Protocolo R2...</span>
              </div>
            ) : (
              <button 
                onClick={handleOrderSubmit} 
                disabled={!config.fileName || !config.width || !config.height} 
                className="w-full mt-24 bg-black text-white p-14 rounded-[4.5rem] font-brand font-black italic uppercase tracking-[0.5em] hover:bg-red-600 transition-all shadow-2xl disabled:opacity-20 flex items-center justify-center space-x-10 group"
              >
                <span>Sincronizar Produção</span> <Zap className="w-8 h-8 group-hover:scale-125 transition-transform text-red-600" />
              </button>
            )}
         </div>
      )}
        ...
        <div className="h-full flex flex-col items-center justify-center text-center py-24 animate-in zoom-in-95">
           ...
           <div className="max-w-4xl w-full bg-black text-white p-16 rounded-[5rem] shadow-2xl border-b-[20px] border-red-600 space-y-12 text-left relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 relative z-10">
                 <div>
                    <span className="text-gray-500 text-[9px] font-black uppercase tracking-widest block mb-2">Target Node</span>
                    <span className="text-[12px] font-black uppercase italic">{hubs.find(h => h.id === config.selectedHubId)?.name || 'Central R2-Unit'}</span>
                 </div>
                 <div>
                    <span className="text-gray-500 text-[9px] font-black uppercase tracking-widest block mb-2">Atomic Spec</span>
                    <span className="text-[12px] font-black uppercase italic">{config.width}x{config.height}{config.unit}</span>
                 </div>
                 <div>
                    <span className="text-gray-500 text-[9px] font-black uppercase tracking-widest block mb-2">Handshake ID</span>
                    <span className="text-[12px] font-black uppercase italic">{config.fileName.slice(0, 15)}...</span>
                 </div>
                 <div>
                    <span className="text-gray-500 text-[9px] font-black uppercase tracking-widest block mb-2">Grid Sync</span>
                    <span className="text-[12px] font-black uppercase italic text-green-500">Verified</span>
                 </div>
              </div>
           </div>
           ...
           <div className="flex flex-col sm:flex-row space-y-6 sm:space-y-0 sm:space-x-8 mt-24">
             ...
             <button className="bg-red-600 text-white px-16 py-8 rounded-[3rem] font-black uppercase text-[12px] tracking-[0.4em] hover:bg-black transition-all shadow-xl flex items-center space-x-6">
                <Activity className="w-5 h-5"/> <span>Abrir Live Telemetry</span>
             </button>
           </div>
        </div>
      )}

      {/* Product Detail Modal Upgrade */}
      {detailProduct && (
              ...
              <div className="p-24 overflow-y-auto space-y-12">
                 ...
                 <div className="grid grid-cols-2 gap-10">
                    <div className="p-8 bg-gray-50 rounded-[3rem] border border-gray-100">
                       <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-4">Precision Level</span>
                       <span className="text-2xl font-brand font-black italic text-black">{detailProduct.specs.precisionLevel}</span>
                    </div>
                    <div className="p-8 bg-gray-50 rounded-[3rem] border border-gray-100">
                       <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-4">Industrial Usage</span>
                       <span className="text-2xl font-brand font-black italic text-black">{detailProduct.specs.usage}</span>
                    </div>
                    <div className="p-8 bg-gray-50 rounded-[3rem] border border-gray-100">
                       <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-4">Durability R2</span>
                       <span className="text-2xl font-brand font-black italic text-black">{detailProduct.specs.durability}</span>
                    </div>
                    <div className="p-8 bg-gray-50 rounded-[3rem] border border-gray-100">
                       <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-4">Output Weight</span>
                       <span className="text-2xl font-brand font-black italic text-black">{detailProduct.specs.weight}</span>
                    </div>
                 </div>
                 ...
                 <div className="p-10 bg-black text-white rounded-[4rem] flex items-center justify-between shadow-2xl">
                    ...
                    <button 
                      onClick={() => { setSelectedProduct(detailProduct); setDetailProduct(null); setStep(2); }} 
                      className="bg-red-600 text-white px-10 py-6 rounded-full font-black uppercase text-[11px] tracking-widest hover:bg-white hover:text-black transition-all shadow-xl"
                    >
                       Configurar Agora
                    </button>
                 </div>
