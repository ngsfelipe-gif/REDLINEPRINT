...
import { ArrowRight, ArrowLeft, CheckCircle2, Cpu, ShieldCheck, Zap, Info, ChevronLeft, ChevronRight, X, Upload, Activity, Package, MessageCircle, Server, FileCheck, FileWarning, Layers, Search, BarChart3, Globe, FileDigit, Monitor, LayoutGrid, Clock, Shield, Settings, Barcode, Mail, User as UserIcon, Lock, Target, Droplets, HardHat, Gauge, Loader2 } from 'lucide-react';
...
interface ProductBuilderProps {
  ...
  language: Language;
  notify?: (title: string, msg: string, type: any) => void;
  onSound?: (type: any) => void;
}

const ProductBuilder: React.FC<ProductBuilderProps> = ({ onAddOrder, user, hubs, products, language, notify, onSound }) => {
  ...
  const handleOrderSubmit = () => {
    ...
    setIsSyncing(true);
    onSound?.('loading');
    notify?.("Injeção Ativa", "Sincronizando metadados com o Cluster R2...", "loading");
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-24">
               <div className="space-y-12">
                  <div className="grid grid-cols-2 gap-10">
                     <div className="space-y-5">
                        ...
                        <div className="flex bg-gray-50 p-6 rounded-[3rem] border border-transparent focus-within:border-red-600 shadow-inner">
                           <input type="number" placeholder="00.0" value={config.width} onChange={e => { onSound?.('click'); setConfig({...config, width: e.target.value}); }} className="bg-transparent flex-grow p-4 font-brand font-black italic text-3xl outline-none" />
                     ...
                     <div className="space-y-5">
                        ...
                        <div className="flex bg-gray-50 p-6 rounded-[3rem] border border-transparent focus-within:border-red-600 shadow-inner">
                           <input type="number" placeholder="00.0" value={config.height} onChange={e => { onSound?.('click'); setConfig({...config, height: e.target.value}); }} className="bg-transparent flex-grow p-4 font-brand font-black italic text-3xl outline-none" />
                           <span className="p-4 font-black uppercase text-gray-300">MM</span>
                        </div>
                     </div>
                  </div>
                  <div className="space-y-5">
                    ...
                    <input type="number" placeholder="100" value={config.quantity} onChange={e => { onSound?.('click'); setConfig({...config, quantity: e.target.value}); }} className="w-full bg-gray-50 p-10 rounded-[3rem] font-brand font-black italic text-3xl outline-none border border-transparent focus:border-red-600 shadow-inner" />
                  </div>
                  <div className="space-y-5">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.4em] ml-8">Industrial Substrate</label>
                    <select value={config.material} onChange={e => { onSound?.('click'); setConfig({...config, material: e.target.value}); }} className="w-full bg-gray-50 p-10 rounded-[3rem] font-black uppercase text-[14px] outline-none border border-transparent focus:border-red-600 shadow-inner appearance-none cursor-pointer">{MATERIALS.map(m => <option key={m}>{m}</option>)}</select>
                  </div>
               </div>
               <div className="space-y-12">
                  ...
                  <div className="space-y-5">
                    ...
                    <select value={config.selectedHubId} onChange={e => { onSound?.('click'); setConfig({...config, selectedHubId: e.target.value}); }} className="w-full bg-gray-50 p-10 rounded-[3rem] font-black uppercase text-[14px] outline-none border border-transparent focus:border-red-600 shadow-inner appearance-none cursor-pointer">
      ...
      {step === 4 && (
        <div className="bg-white rounded-[5rem] shadow-2xl border-t-[20px] border-red-600 p-32 text-center space-y-16 animate-in zoom-in-95 duration-700 max-w-4xl mx-auto">
           <div className="relative inline-block">
              ...
              <div className="absolute inset-0 bg-green-500/20 blur-3xl rounded-full status-pulse" />
           </div>
