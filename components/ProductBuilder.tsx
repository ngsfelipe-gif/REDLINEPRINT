...
const ProductBuilder: React.FC<ProductBuilderProps> = ({ onAddOrder, user, hubs, products, language }) => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-10 py-12 industrial-grid min-h-screen pt-40 lg:pt-48">
      {step === 1 && (
         <div className="space-y-24 animate-in fade-in">
            <div className="flex flex-col xl:flex-row justify-between items-end gap-12">
               <div className="space-y-10">
                 <div className="flex items-center space-x-3 bg-black text-white px-5 py-2 rounded-full w-fit shadow-lg">
                 ...
                 <h3 className="text-8xl font-brand font-black italic uppercase text-black tracking-tighter leading-[0.8] mb-4">Módulos <br/><span className="text-red-600">Industriais.</span></h3>
                 <div className="flex overflow-x-auto gap-4 pb-6 scrollbar-hide">
                    {categories.map(cat => (
                      <button key={cat} onClick={() => { setActiveCategory(cat); setCurrentPage(1); }} className={`px-10 py-4 rounded-full text-[11px] font-black uppercase tracking-widest whitespace-nowrap transition-all border-2 ${activeCategory === cat ? 'bg-red-600 text-white border-red-600 shadow-xl scale-105' : 'bg-white text-gray-400 border-gray-100 hover:border-black hover:text-black'}`}>{cat}</button>
                    ))}
                 </div>
               </div>
               <div className="flex flex-col items-end gap-8 w-full max-w-xl">
                  <div className="flex items-center space-x-8 text-[11px] font-black uppercase tracking-widest text-gray-400 mb-2">
                     <div className="flex items-center space-x-2"><Monitor className="w-4 h-4 text-red-600" /> <span>{products.length} Assets</span></div>
                     <div className="flex items-center space-x-2"><Activity className="w-4 h-4 text-red-600" /> <span>Sync: 0.2ms</span></div>
                  </div>
                  <div className="bg-white p-3 rounded-[3rem] border border-gray-100 shadow-2xl flex items-center w-full focus-within:border-black transition-all">
                     <Search className="w-8 h-8 text-gray-300 ml-6" />
                     <input type="text" placeholder="LOCALIZAR NO GRID..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="bg-transparent flex-grow outline-none font-black uppercase text-[13px] p-6 placeholder:text-gray-200" />
                  </div>
               </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-12">
               {paginatedProducts.map(p => (
                 <div key={p.id} onClick={() => setSelectedProduct(p)} className={`bg-white border-2 rounded-[4.5rem] p-10 transition-all cursor-pointer relative group ${selectedProduct?.id === p.id ? 'border-red-600 shadow-[0_50px_100px_rgba(204,0,0,0.1)] scale-[1.03]' : 'border-gray-50 hover:border-black shadow-lg'}`}>
                    {p.badge && <div className="absolute top-10 right-10 bg-black text-white px-5 py-2 rounded-full text-[9px] font-black z-10 shadow-lg tracking-widest">{p.badge}</div>}
                    <div className="w-full aspect-square rounded-[3.5rem] overflow-hidden mb-12 shadow-inner bg-gray-50 relative">
                       ...
                       <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center space-x-3 opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0">
                          <button onClick={(e) => { e.stopPropagation(); setDetailProduct(p); }} className="bg-white/90 backdrop-blur-md p-4 rounded-2xl text-black hover:bg-red-600 hover:text-white transition-all shadow-xl"><Info className="w-6 h-6" /></button>
                       </div>
                    </div>
                    <div className="space-y-6 px-2">
                       <div>
                          <span className="text-[10px] font-black uppercase text-red-600 tracking-[0.4em] mb-2 block">{p.category}</span>
                          <h5 className="text-4xl font-brand font-black italic uppercase text-black leading-tight">{p.name}</h5>
                       </div>
                       <p className="text-[13px] font-medium text-gray-500 italic line-clamp-2 leading-relaxed h-10">{p.description}</p>
                       <div className="grid grid-cols-2 gap-6 pt-8 border-t border-gray-100">
                          <div className="flex items-center space-x-3">
                             <Clock className="w-4 h-4 text-red-600" />
                             <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">ETD: 24H</span>
                          </div>
                          <div className="flex items-center space-x-3">
                             <Shield className="w-4 h-4 text-red-600" />
                             <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">CERT: ISO</span>
                          </div>
                       </div>
                       <div className="flex justify-between items-center mt-12 pt-8">
                          {user || isAdmin || isB2B ? <span className="text-5xl font-brand font-black italic text-black">€{p.basePrice}<span className="text-[11px] font-normal opacity-30 italic">/{p.unit}</span></span> : <span className="text-[10px] font-black uppercase text-gray-300 italic tracking-[0.2em]">LOGIN PARA PREÇO</span>}
                          <div className={`p-6 rounded-3xl transition-all ${selectedProduct?.id === p.id ? 'bg-red-600 text-white animate-pulse shadow-xl shadow-red-600/30 scale-110' : 'bg-gray-100 text-gray-300 group-hover:bg-black group-hover:text-white group-hover:rotate-12'}`}><Zap className="w-8 h-8" /></div>
                       </div>
                    </div>
                 </div>
               ))}
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center space-x-12 mt-20 py-16 border-t border-gray-100">
                 <button onClick={() => { setCurrentPage(prev => Math.max(prev - 1, 1)); window.scrollTo({top: 0, behavior: 'smooth'}); }} disabled={currentPage === 1} className="p-8 bg-white border border-gray-100 rounded-full disabled:opacity-20 hover:bg-black hover:text-white transition-all shadow-xl"><ChevronLeft className="w-10 h-10" /></button>
                 <div className="flex items-center space-x-5">
                    {Array.from({ length: totalPages }).map((_, i) => (
                      <button key={i} onClick={() => { setCurrentPage(i + 1); window.scrollTo({top: 0, behavior: 'smooth'}); }} className={`w-16 h-16 rounded-[2rem] font-brand font-black italic text-2xl transition-all ${currentPage === i + 1 ? 'bg-red-600 text-white shadow-2xl scale-125' : 'bg-gray-100 text-gray-400 hover:bg-black hover:text-white'}`}>{i + 1}</button>
                    ))}
                 </div>
                 <button onClick={() => { setCurrentPage(prev => Math.min(prev + 1, totalPages)); window.scrollTo({top: 0, behavior: 'smooth'}); }} disabled={currentPage === totalPages} className="p-8 bg-white border border-gray-100 rounded-full disabled:opacity-20 hover:bg-black hover:text-white transition-all shadow-xl"><ChevronRight className="w-10 h-10" /></button>
              </div>
            )}

            {selectedProduct && <button onClick={() => setStep(2)} className="w-full bg-black text-white p-14 rounded-[4.5rem] font-brand font-black italic uppercase tracking-[0.6em] text-2xl mt-12 hover:bg-red-600 transition-all shadow-2xl flex items-center justify-center animate-in slide-in-from-bottom-5">Configurar Módulo R2 <ArrowRight className="ml-10 w-10 h-10 animate-pulse" /></button>}
         </div>
      )}
