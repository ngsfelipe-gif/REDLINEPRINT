...
import { 
  ...
  Eye, Plus, Edit2, Trash2, Globe, Server, Users, Activity, BarChart3, Clock, Check
} from 'lucide-react';
...
const Backoffice: React.FC<BackofficeProps> = ({ orders, hubs, user, onUpdateStatus, onAddHub, onUpdateHub }) => {
  ...
  // Filtragem lógica baseada em permissões
  const myHubs = isAdmin ? hubs : hubs.filter(h => h.ownerId === user?.id);
  const myHubIds = myHubs.map(h => h.id);
  
  const visibleOrders = isAdmin 
    ? orders 
    : orders.filter(o => myHubIds.includes(o.nodeId));

  const pendingOrders = visibleOrders.filter(o => o.status === 'Pendente Aprovação Hub');
  ...
  if (!isB2B) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-40 text-center animate-in zoom-in">
         ...
         <p className="text-[12px] font-black uppercase text-gray-400 mt-6 tracking-[0.5em]">Credenciais Administrativas R2 Requeridas</p>
      </div>
    );
  }

  const handleAcceptOrder = (id: string) => {
    onUpdateStatus(id, 'Orçamento Gerado');
  };

  const handleCreateHub = () => {
    const newHub: PartnerNode = {
      id: `NODE-${Math.floor(Math.random() * 900) + 100}`,
      name: 'Novo Hub Industrial',
      ...
      description: 'Nodo fabril registado por parceiro B2B. Aguarda auditoria R2.',
      ownerId: user?.id || ''
    };
    onAddHub(newHub);
  };
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end mb-16 gap-10">
        <div className="animate-in slide-in-from-left-6">
          <h2 className="text-6xl md:text-8xl font-brand font-black italic uppercase tracking-tighter text-black leading-none">
            {isAdmin ? 'Controlo' : 'Pipeline'} <br /> <span className="text-red-600">Industrial.</span>
          </h2>
          <div className="flex items-center space-x-4 mt-8">
            <div className="bg-black text-white px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center shadow-xl">
               <ShieldCheck className="w-3 h-3 mr-2 text-red-600" /> {user?.role} Mode Active
            </div>
            {pendingOrders.length > 0 && (
              <div className="bg-red-600 text-white px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest animate-pulse">
                {pendingOrders.length} Ordens Pendentes
              </div>
            )}
          </div>
        </div>

        <div className="flex flex-wrap bg-white p-2 rounded-[2rem] shadow-2xl border border-gray-100 gap-2">
          ...
          <button onClick={() => setActiveView('hubs')} className={`px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'hubs' ? 'bg-black text-white shadow-xl' : 'text-gray-400 hover:bg-gray-50'}`}>Meus Hubs</button>
          {isAdmin && <button onClick={() => setActiveView('users')} className={`px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === 'users' ? 'bg-black text-white shadow-xl' : 'text-gray-400 hover:bg-gray-50'}`}>Usuários B2B</button>}
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-16">
        <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm flex flex-col justify-between hover:shadow-xl transition-all">
           ...
           <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Jobs Totais</span>
        </div>
        <div className="bg-black text-white p-10 rounded-[3rem] shadow-2xl flex flex-col justify-between border-b-[15px] border-red-600">
           <Clock className="w-10 h-10 text-red-600 mb-6" />
           <span className="text-6xl font-brand font-black italic">{pendingOrders.length}</span>
           <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Aguardando Aceitação</span>
        </div>
        <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm flex flex-col justify-between">
           ...
           <span className="text-6xl font-brand font-black italic">{myHubs.length}</span>
           <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Capacidade Ativa</span>
        </div>
        <div className="bg-red-600 text-white p-10 rounded-[3rem] shadow-2xl flex flex-col justify-between">
           ...
           <span className="text-4xl font-brand font-black italic uppercase leading-none">Sync Ativo</span>
           <span className="text-[10px] font-black text-red-100 uppercase tracking-widest">Node FRA-Lisboa</span>
        </div>
      </div>
        ...
        {activeView === 'orders' && (
          <div className="animate-in fade-in duration-500">
            <div className="p-12 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
               <h3 className="text-3xl font-brand font-black italic uppercase text-black">Matriz de Produção</h3>
               <div className="flex space-x-3">
                  <div className="flex items-center space-x-2 bg-green-50 text-green-600 px-4 py-2 rounded-xl text-[9px] font-black uppercase border border-green-100">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-black text-white text-[10px] font-black uppercase tracking-[0.2em]">
                  <tr>
                    <th className="px-12 py-8">ID Ordem</th>
                    <th className="px-8 py-8">Cliente Industrial</th>
                    <th className="px-8 py-8">Asset / Produto</th>
                    <th className="px-8 py-8">Status R2</th>
                    <th className="px-8 py-8">Hub</th>
                    <th className="px-12 py-8 text-right">Ações</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {visibleOrders.map(order => (
                    <tr key={order.id} className={`group hover:bg-gray-50/80 transition-all ${order.status === 'Pendente Aprovação Hub' ? 'bg-red-50/30' : ''}`}>
                      ...
                      <td className="px-8 py-10">
                        {order.status === 'Pendente Aprovação Hub' ? (
                          <div className="flex items-center space-x-4">
                            <button 
                              onClick={() => handleAcceptOrder(order.id)}
                              className="bg-green-600 text-white p-3 rounded-xl hover:bg-black transition-all shadow-lg"
                              title="Aceitar Encomenda"
                            >
                              <Check className="w-4 h-4" />
                            </button>
                            <button 
                              className="bg-gray-200 text-gray-500 p-3 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-lg"
                      ...
                      <td className="px-8 py-10">
                         <span className="px-3 py-1 bg-red-50 text-red-600 rounded-lg text-[10px] font-black uppercase border border-red-100">{order.nodeId}</span>
                      </td>
        ...
        {activeView === 'hubs' && (
          <div className="p-12 animate-in fade-in duration-500">
             <div className="flex justify-between items-center mb-16">
                <div>
                   <h3 className="text-4xl font-brand font-black italic uppercase leading-none">Gestão de <span className="text-red-600">Nodos B2B.</span></h3>
                   <p className="text-[11px] font-black text-gray-400 uppercase mt-2 tracking-widest">Administração de infraestrutura física e capacidade</p>
                </div>
                <button onClick={handleCreateHub} className="bg-red-600 text-white px-10 py-5 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-black transition-all shadow-xl flex items-center space-x-4">
                   <Plus className="w-5 h-5" /> <span>Adicionar Unidade Fabril</span>
                </button>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {myHubs.map(hub => (
                  <div key={hub.id} className="bg-gray-50 rounded-[3rem] overflow-hidden border border-gray-100 group hover:border-red-600 transition-all shadow-sm hover:shadow-2xl">
                     ...
                     <div className="p-10">
                        <div className="flex justify-between items-start mb-8">
                           ...
                           <div className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${
                             hub.status === 'Online' ? 'bg-green-500 text-white' : 
                             hub.status === 'Aguardando Validação' ? 'bg-yellow-500 text-white' : 'bg-orange-500 text-white'
                           }`}>{hub.status}</div>
                        </div>
